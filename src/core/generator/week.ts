...
// Клетчатка — мягкий минимум: kbju-master задаёт её как штрафную (soft), а не
// жёсткую границу, и в отжиге это отражено (minPenalty, не rangePenalty). Вердикт
// `compromised` приводим к той же трактовке: небольшой недобор при выполненных
// ЖЁСТКИХ коридорах КБЖУ (ккал/Б/Ж/У) не помечает неделю неудачной — иначе на
// реалистичной базе (тикет 21, ~60 рецептов) пользователь видел бы ложное
// предупреждение из-за 1–2 г клетчатки при идеальных макросах. Сама величина
// допуска (3 г) — инженерная эвристика этого слоя, а не число из kbju-master;
// грубый недобор (> допуска) неделю по-прежнему компрометирует.

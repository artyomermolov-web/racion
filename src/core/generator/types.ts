...
export interface GeneratorRecipe {
  ...
  keywords?: string[];
  /**
   * Персонализация базового рецепта (тикет 17, decision 04): если задан — это
   * своя версия пользователя, которая ВЫТЕСНЯЕТ базовый рецепт с этим id из пула
   * кандидатов (см. `resolvePersonalization`). У базовых и обычных кастом-рецептов
